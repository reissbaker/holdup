/*
 * Note: you're currently returning objects with much greater capabilities
 * than the Promises/A and Promises/A+ spec. These should probably be renamed
 * to `Deferred`s, and you should make a simple `Promise` object that wraps a
 * `Deferred` and exposes a single `then` method. Maybe just add a `toPromise`
 * method on `Deferred` that returns something like:
 *     
 *     { then: function(f,r) { that.then(f, r); } }
 *
 * Or keep the `Deferred`s named `Promise`, and just have a `toPublic` method
 ...
 * Promises/A+, your promises can forward returned promises -- allowing
...
!function() {
   ...
   *
   * The return promise will call its `then` callback with whatever is passed
  ...
  root.make = function(callback) {
    ...
    callback(bind(promise.fulfill, promise), bind(promise.reject, promise));
    return promise;
  };
