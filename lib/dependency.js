 ...
 * than the Deferreds/A and Deferreds/A+ spec. These should probably be renamed
 * to `Deferred`s, and you should make a simple `Deferred` object that wraps a
 * `Deferred` and exposes a single `then` method. Maybe just add a `toDeferred`
 * method on `Deferred` that returns something like:
 *     
 *     { then: function(f,r) { that.then(f, r); } }
 *
 * Or keep the `Deferred`s named `Deferred`, and just have a `toPublic` method
