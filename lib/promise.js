...
!function() {
  ...
  Promise.prototype.then = function(callback, errback) {
    ...
    else if(isRejected(this)) resolveLater(this, promise, REJECTED, errback);
    else resolveLater(this, promise, FULFILLED, callback);
  ...
  function resolveLater(scope, promise, state, callback) {
    later(function() { resolve(scope, promise, state, callback); });
  }
  ...
  function bufferPromise(scope, promise) {
    // Use `resolve` instead of `resolveLater` -- buffers are already async,
    // don't need to force async again.
